...
contract("IdleCompoundETH", function ([_, creator, nonOwner, someone, foo]) {
    beforeEach(async function () {
        ...
        this.cWETHMock = await cWETHMock.new(this.WETHMock.address, creator, this.WhitePaperMock.address, {
            from: creator,
        });
        this.cWETHWrapper = await IdleCompoundETH.new(this.cWETHMock.address, this.WETHMock.address, nonOwner, {
            from: creator,
        });
    });
    ...
    it("returns next supply rate given amount", async function () {
        ...
        const expectedRes = a
            .add(b.mul(c).div(b.add(s).add(x)))
            .div(k)
            .mul(e)
            .mul(b)
            .div(s.add(x).add(b).sub(d))
            .div(j)
            .mul(k)
            .mul(f); // to get the yearly rate
